...
# Convert HTML source code to string
...
try:
  os.mkdir('course-analytics')
except:
  print("Course-analytics directory already exists.")

for a in range(len(data_list)):

  json_path = data_list[a]
  course_code_first = (json_path.split("_"))[1][:-5]
  print(course_code_first)

  try:
    os.mkdir("course-analytics/" + course_code_first.lower())
  except:
    print(course_code_first + " directory already exists.")

  # Course JSON File
  course_json_file = "data/data_" + course_code_first + ".json"
  ...
  faculties_short = ["FENS","FASS","SOM","SL","CIAD"]
  faculties_long = ["Faculty of Engineering and Natural Sciences", "Faculty of Arts and Social Sciences", "Sabancı Business School", "School of Languages", "Center for Individual and Academic Development"]
  ...
  if len(course_prerequisites) == 0:
    ...
    for prerequisite in course_prerequisites:
      prerequisite_link = prerequisite.replace(" ", "").lower()
  ...
  with open("course-analytics/" + course_code_first.lower() + "/index.html", "w", encoding="utf-8") as file:
